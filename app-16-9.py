...
CUSTOM_AUDIO_FILES = {
    ...
    # Correct answer has its own dedicated file/slot now, independent of
    # start_play -- all three prize tiers (small/medium/big) share this
    # one file, since KBC-style shows use one "correct" sting regardless
    # of the amount, just with a bigger celebration visually.
    "correct_small": "correct_answer.mp3",
    "correct_medium": "correct_answer.mp3",
    "correct_big": "correct_answer.mp3",
}
AMBIENT_TICKTICK_FILE = "ticktick.mp3"       # loops while the timer is live -- the ONLY looping audio in the app

# ---- All audio slots the admin panel can manage, in one place. ----
# Each entry: (cue_key, filename, human label). Used to render the upload
# section and status list on the admin panel.
ALL_AUDIO_SLOTS = [
    ("lobby_intro", "kbc_intro_main.mp3", "🏠 Splash Screen Intro (plays once, via Play Intro button)"),
    ("start_play", "intro_start_play.mp3", "▶️ Begin Question 1"),
    ("ticktick", "ticktick.mp3", "⏱️ Timer Tick-Tick (loops during countdown)"),
    ("lock", "lock_option.mp3", "🔒 Lock Answer"),
    ("correct", "correct_answer.mp3", "✅ Correct Answer"),
    ("next_question", "next_question.mp3", "➡️ Next Question"),
    ("wrong", "wrong_answer.mp3", "❌ Wrong Answer"),
]


def audio_file_status(filename: str):
    """Returns (state, size_bytes) for a given assets filename.
    state is one of: 'missing', 'broken' (exists but under 1KB, i.e. a
    failed upload like the 2-byte files from before), or 'ok'."""
...
def save_uploaded_audio(filename: str, uploaded_file) -> int:
    """Writes an uploaded file's bytes to assets/<filename>, creating the
    assets folder if needed. Returns the number of bytes written so the
    caller can immediately confirm it's not another 0/2-byte failure."""
...
def _load_audio_b64_cached(filename: str, mtime: float):
    """Actual file read + base64 encode, cached by (filename, mtime) so a
    re-uploaded file (which changes mtime) automatically busts the old
    cached entry -- no stale audio after a re-upload."""
    path = os.path.join(ASSETS_DIR, filename)
...
def _load_audio_b64(filename: str):
    """Read an audio file from the assets folder and return it as a cached
    base64 data URI so we don't re-read/re-encode multi-MB files on every
    rerun. Returns None if the file isn't there (or is a 0-byte/broken
    upload), so missing/broken assets degrade gracefully instead of
    crashing the app or playing silence pretending to be audio."""
    path = os.path.join(ASSETS_DIR, filename)
    if not os.path.exists(path):
        return None
    size = os.path.getsize(path)
    if size < 1024:  # anything under 1KB is not a real mp3 -- treat as broken/missing
        return None
    mtime = os.path.getmtime(path)
    return _load_audio_b64_cached(filename, mtime)
...
OPTION_REVEAL_GAP = 1.5  # seconds between each option (A, B, C, D) auto-appearing
...
DEFAULT_STATE = {
    ...
    "contestant_name": "",
    "final_amount": 0,
    "intro_played": False,  # host plays the splash-screen intro exactly once via the admin "Play Intro" button; it then hides itself
    "question_revealed": False,  # host must press "Show Question" before the question text appears; the empty question box shows first
    "revealed_options": [],  # indices of options shown so far -- host reveals each individually via "Show A/B/C/D" buttons
    "options_reveal_started_at": None,  # kept for backward-compat state shape; no longer drives auto-reveal timing
    "celebration_tier": None,  # "small" | "medium" | "big" -- set on a correct answer, drives visual/audio intensity
    "timer_seconds": 0,
    "timer_started_at": None,
    "sound_cue": None,
    "sound_cue_id": 0,
    "preview_file": None,       # filename to preview (admin "Test" button), independent of game cues
    "preview_loop_seconds": None,  # if set, play this file on loop for N seconds then stop (for ambient/loop previews)
    "preview_id": 0,
...
def advance_auto_reveal_and_timer(state: dict) -> dict:
    """Auto-starts the countdown timer the instant all 4 options are
    revealed (the host now reveals each option A/B/C/D individually via
    its own button, rather than a timed auto-cascade -- see the admin
    panel's "Show Option X" buttons). Called on every read from both
    admin and display so the two stay in sync without needing a
    background job. Persists the computed values so the "timer
    auto-started" transition only fires once.
    """
    revealed_options = state.get("revealed_options", [])
    all_four_shown = len(revealed_options) >= 4
    ...
    if all_four_shown and timer_not_started_yet and duration > 0 and state.get("status") == "question":
        # Auto-start the countdown the instant the last option (D) is
        # revealed, matching real KBC's clock behavior.
...
def trigger_preview(filename: str, loop_seconds: int = None):
    """Broadcast a 'preview this audio file' event through shared state so
    BOTH the admin panel and the display screen play it -- this is
    separate from trigger_sound because previewing a file shouldn't touch
    any real game state (status, revealed options, etc.). loop_seconds set
    means play on loop for that many seconds then stop (used for the
    lobby-intro and ticktick loop previews); None means play once through."""
    state = get_state()
    patch = {
        "preview_file": filename,
        "preview_loop_seconds": loop_seconds,
        "preview_id": state.get("preview_id", 0) + 1,
    }
    return set_state(patch)
...
def play_cue(cue_name: str, cue_id: int):
    ...
    if data_uri:
        ...
        (function() {{
            try {{
                // Use window.top.Audio, not this iframe's own Audio -- the
                // "Tap to Enable Sound" gate unlocks autoplay on the TOP
                // window (window.top), since that's the one persistent
                // frame across all of Streamlit's fragment reruns. An
                // Audio object built in this throwaway iframe would be
                // subject to ITS OWN (never-unlocked) autoplay policy.
                const AudioCtor = (window.top && window.top.Audio) || window.Audio;
                const audio = new AudioCtor("{data_uri}");
                audio.volume = 1.0;
    ...
    (function() {{
        try {{
            ...
            const AudioContextCtor = topWin.AudioContext || topWin.webkitAudioContext || window.AudioContext || window.webkitAudioContext;
...
def play_file_preview(filename: str, preview_id: int, loop_seconds: int = None):
    """Renders an invisible component that plays a raw audio file once,
    identified only by filename (not tied to a game cue). Used by the
    admin panel's 'Test' buttons so the host can preview an uploaded file
    on both the admin panel and display screen without touching any real
    game state. If loop_seconds is set, loops the file and auto-stops
    (with a quick fade) after that many seconds -- used for previewing
    the lobby-intro and ticktick loops without leaving them running."""
    data_uri = _load_audio_b64(filename)
    if not data_uri:
        return
    loop_js = "true" if loop_seconds else "false"
    stop_js = ""
    if loop_seconds:
        stop_js = f"""
        setTimeout(function() {{
            const fadeStep = () => {{
                ...
                if (audio.volume > 0) {{ setTimeout(fadeStep, 40); }}
                else {{ audio.pause(); }}
            }};
            fadeStep();
        }}, {loop_seconds * 1000});
        """
    html = f"""
    <div style="display:none" data-preview-id="{preview_id}"></div>
    <script>
    (function() {{
        try {{
            const AudioCtor = (window.top && window.top.Audio) || window.Audio;
            ...
            audio.loop = {loop_js};
            audio.volume = 0.9;
            audio.play().catch(e => console.log('Preview autoplay blocked:', e));
            {stop_js}
        }} catch (e) {{
            console.log('Preview skipped:', e);
        }}
    }})();
    </script>
    """
...
def play_ambient_loop(active: bool, tension: str = "low"):
    ...
    IMPORTANT: this only calls components.html() when `active` actually
    CHANGES (tracked in st.session_state), not on every 0.35s poll tick.
    Re-embedding a multi-MB base64 audio blob into a fresh iframe every
    tick is what caused the page to bog down/go dark during a live
    question -- the browser was accumulating hundreds of heavy iframes
    per minute. Now the <audio> element is created once per loop-start and
    left alone; only a stop/retune message needs a fresh (tiny) component
    call. tension affects pitch/brightness for the synthesized fallback,
    and playback rate for the real file: 'low' during Q1-5, 'mid' during
    Q6-10, 'high' during Q11-15 (subtle, ambient -- not meant to be loud
    or distracting).
    """
    tension_settings = {
    ...
    prev_active = st.session_state.get("_ambient_active", False)
    prev_tension = st.session_state.get("_ambient_tension", None)
    # Only re-render the component when something actually changed:
    # start, stop, or a tension-tier change while already playing.
    if active == prev_active and (not active or tension == prev_tension):
        return
    st.session_state["_ambient_active"] = active
    st.session_state["_ambient_tension"] = tension if active else None
    ...
    if ticktick_uri:
        ...
        (function() {{
            try {{
                // State lives on window.top (the real persistent page), not
                // this throwaway iframe's own window -- both so autoplay
                // permission applies (see render_sound_unlock_overlay) and
                // so "already playing" is correctly detected the next time
                // this fires from a brand new iframe.
                const topWin = window.top || window;
                const AudioCtor = topWin.Audio || window.Audio;
                if (!topWin.__sfqTicktick) {{ topWin.__sfqTicktick = {{ audio: null, playing: false, starting: false }}; }}
                const state = topWin.__sfqTicktick;
                ...
                // .starting closes the race window between checking and
                // setting playing=true (same fix as the lobby-intro/suspense
                // loops -- see their docstrings for why session_state alone
                // can't be trusted as the "already playing" guard).
                if (state.starting) return;
                state.starting = true;

                const audio = new AudioCtor("{ticktick_uri}");
                audio.loop = true;
    ...
    (function() {{
        try {{
            const topWin = window.top || window;
            const AudioContextCtor = topWin.AudioContext || topWin.webkitAudioContext || window.AudioContext || window.webkitAudioContext;
            if (!topWin.__sfqAmbient) {{ topWin.__sfqAmbient = {{ ctx: null, nodes: [], playing: false }}; }}
            const state = topWin.__sfqAmbient;
            const shouldPlay = {str(active).lower()};
            ...
            const ctx = state.ctx || new AudioContextCtor();
            state.ctx = ctx;
            const base = {cfg['base']};
...
/* ---- Fixed-frame layout for TV / tablet: everything fits in one
   viewport height with no scrolling, using vh-based sizing and clamp()
   so text/spacing shrink gracefully on shorter screens instead of
   overflowing. This is the display screen's whole reason for existing --
   it's meant to be watched on a big screen from across a room, not
   scrolled through like a phone page. ---- */
html, body { overflow: hidden; height: 100%; }
.stApp { height: 100vh; overflow: hidden; }
.block-container {
    padding-top: 0.6vh; padding-bottom: 0.6vh;
    padding-left: clamp(0.8rem, 2vw, 2.4rem); padding-right: clamp(0.8rem, 2vw, 2.4rem);
    max-width: 1100px;
...
/* Streamlit wraps every st.markdown/st.button/st.columns call in its own
   element/vertical-block container carrying its own default margin/gap.
   Left alone, those add up to real vertical space across ~8-10 stacked
   elements (title, header row, lifeline dock, timer, question, 4
   options) -- exactly what was pushing option D below the fold. Zeroing
   these out is what actually makes the clamp()/vh sizing above add up to
   a true single-screen fit instead of still overflowing by a few hundred
   px of accumulated default spacing. */
...
div[data-testid="stHorizontalBlock"] { gap: 0.6rem !important; align-items: center; }
.stMarkdown { line-height: 1; }

/* ---- Title ---- */
.sfq-title {
    text-align: center; font-family: 'Fraunces', serif; font-weight: 600;
    font-size: clamp(1.2rem, 3.4vh, 2.1rem); letter-spacing: 0.01em; color: var(--sfq-ink);
    margin-bottom: 0; line-height: 1.1;
    flex: 0 0 auto;
}
.sfq-title .accent { color: var(--sfq-gold); font-style: italic; }
.sfq-subtitle {
    text-align: center; color: var(--sfq-gold); font-size: clamp(0.7rem, 1.6vh, 0.95rem); font-weight: 500;
    margin-bottom: 0; min-height: 0; letter-spacing: 0.02em;
    flex: 0 0 auto;
}
...
.sfq-header-name {
    font-size: clamp(0.65rem, 1.6vh, 0.92rem); color: var(--sfq-ink-dim); font-weight: 500;
...
.sfq-prize-badge {
    ...
    padding: clamp(0.2rem, 0.8vh, 0.5rem) 0.9rem; text-align: center;
}
.sfq-prize-badge .label {
    font-size: clamp(0.5rem, 1.1vh, 0.6rem); color: var(--sfq-ink-dim); letter-spacing: 0.1em; text-transform: uppercase;
}
.sfq-prize-badge .amount {
    font-family: 'Fraunces', serif; font-size: clamp(0.85rem, 2vh, 1.15rem); font-weight: 600; color: var(--sfq-gold);
}
...
.sfq-qnumber {
    text-align: center; color: var(--sfq-gold); font-weight: 600; font-size: clamp(0.6rem, 1.4vh, 0.8rem);
    letter-spacing: 0.14em; text-transform: uppercase; margin-bottom: 0.4vh;
    flex: 0 0 auto;
}
.sfq-question-card {
    background: var(--sfq-panel-light);
    ...
    border: 1px solid var(--sfq-glass-border);
    border-radius: 22px; padding: clamp(0.7rem, 2.2vh, 1.6rem) clamp(1rem, 3vw, 2rem);
...
.sfq-question-text {
    font-family: 'Fraunces', serif; font-size: clamp(0.95rem, 2.6vh, 1.5rem); font-weight: 500;
    line-height: 1.35; color: var(--sfq-ink);
}
.sfq-question-text.sfq-question-waiting {
    color: var(--sfq-ink-dim); font-style: italic; font-size: clamp(0.85rem, 2vh, 1.2rem);
    animation: sfq-waiting-pulse 1.8s ease-in-out infinite;
}
@keyframes sfq-waiting-pulse {
    0%, 100% { opacity: 0.55; }
    50% { opacity: 1; }
}
...
.sfq-option {
    ...
    border-radius: 14px; padding: clamp(0.4rem, 1.3vh, 0.9rem) 1.2rem; margin-bottom: clamp(0.3rem, 0.9vh, 0.6rem);
    font-size: clamp(0.8rem, 1.9vh, 1.02rem); font-weight: 500; color: var(--sfq-ink);
...
.sfq-option-letter {
    ...
    width: clamp(22px, 4vh, 30px); height: clamp(22px, 4vh, 30px); border-radius: 50%;
    display: flex; align-items: center; justify-content: center;
    font-weight: 700; font-size: clamp(0.7rem, 1.6vh, 0.85rem); flex-shrink: 0;
...
.sfq-ladder-panel {
    ...
    /* Positioned as an overlay (not part of the normal flex stack) so
       opening the milestone ladder never pushes the question/options
       below the fold on the fixed TV/tablet frame -- it floats on top
       of the game instead. */
    position: fixed; top: 8vh; right: 2vw; z-index: 500;
    width: min(320px, 44vw); max-height: 80vh; overflow-y: auto;
...
.sfq-lifeline-dock { display: flex; gap: 0.6rem; justify-content: center; margin: 0.4vh 0 0.4vh; flex: 0 0 auto; }
.sfq-lifeline-chip {
    background: var(--sfq-panel-light); backdrop-filter: blur(14px);
    border: 1px solid var(--sfq-glass-border); border-radius: 50px;
    padding: clamp(0.25rem, 0.7vh, 0.5rem) 0.9rem; display: flex; align-items: center; gap: 0.4rem;
    font-size: clamp(0.65rem, 1.5vh, 0.85rem); font-weight: 500; color: var(--sfq-ink);
...
.sfq-phone-bubble {
    ...
    padding: clamp(0.5rem, 1.5vh, 1.1rem) 1.4rem; margin: 0.5vh 0;
    font-size: clamp(0.75rem, 1.8vh, 1rem); font-style: italic; color: var(--sfq-ink); text-align: center;
    animation: sfq-rise-in 0.5s cubic-bezier(0.22, 1, 0.36, 1) both;
    flex: 0 0 auto;
}

/* ---- Suspense dots ---- */
.sfq-suspense-wrap { text-align: center; padding: 0.6rem; }
.sfq-suspense-text {
    font-size: clamp(0.75rem, 1.7vh, 1rem); font-weight: 600; letter-spacing: 0.1em; color: var(--sfq-gold);
...
/* ---- Timer ring ---- */
.sfq-timer-wrap { display: flex; justify-content: center; margin-bottom: 0.5vh; animation: sfq-rise-in 0.4s both; flex: 0 0 auto; }
.sfq-timer-box { position: relative; width: clamp(48px, 9vh, 78px); height: clamp(48px, 9vh, 78px); }
.sfq-timer-box svg { transform: rotate(-90deg); width: 100%; height: 100%; }
...
.sfq-timer-number {
    ...
    font-family: 'Fraunces', serif; font-size: clamp(0.9rem, 2.2vh, 1.4rem); font-weight: 600; color: var(--sfq-gold);
}
...
/* ---- Result banners ---- */
.sfq-result-banner {
    text-align: center; padding: clamp(1.2rem, 4vh, 2.4rem) 1.6rem; border-radius: 24px;
    font-family: 'Fraunces', serif; font-size: clamp(1.1rem, 3.4vh, 1.7rem); font-weight: 600; margin: 0;
    backdrop-filter: blur(18px);
...
/* Note: the "Tap to Enable Sound" gate is NOT styled here -- it's built
   and styled dynamically by render_sound_unlock_overlay()'s JS, which
   injects its own <style> tag directly into window.top.document. It has
   to work that way because that overlay is created by a components.html()
   iframe reaching into the real top-level page, not by this CSS block
   (which only applies inside Streamlit's own DOM, not a dynamically
   inserted cross-frame element). See that function's docstring. */
</style>
"""
...
def render_sound_unlock_overlay():
    """A full-screen 'Tap to Enable Sound' overlay for the display screen.

    Two real browser constraints drove this design, both confirmed rather
    than assumed:
    1. <script> tags inside st.markdown(unsafe_allow_html=True) never
       execute -- Streamlit strips/ignores them. Only components.html()
       (which renders inside a real iframe) actually runs JavaScript.
    2. A components.html() iframe is its own frame -- an overlay <div>
       positioned "fixed" inside it only covers that iframe's own bounds,
       not the actual browser window. So instead of drawing the overlay
       INSIDE the iframe, this component's script reaches into
       window.top.document (the real top-level page) and builds the
       overlay there. Streamlit's component iframes carry the
       allow-same-origin sandbox flag, which is what makes this
       cross-frame DOM access permitted.

    On tap, this ONLY unlocks the browser's autoplay permission (via a
    silent dummy Audio object + resuming a WebAudio AudioContext) -- it
    does NOT play any specific track. The splash-screen intro is played
    exclusively by the host's explicit "▶️ Play Intro" button in the admin
    panel, exactly once, with no automatic playback anywhere. Keeping
    "unlock audio" and "play the intro" as two separate, deliberate
    actions is what guarantees the intro can never play on its own.

    Because the flag (__sfqSoundUnlocked) and the gate element both live
    on window.top, they persist across every later components.html() call
    the fragment makes, even though each of those is a brand new, separate
    iframe.
    """
    return f"""
    <script>
    (function() {{
        try {{
            ...
            if (topWin.__sfqSoundUnlocked) return;  // already unlocked, nothing to do

            if (!topDoc.getElementById('sfq-sound-gate')) {{
                const style = topDoc.createElement('style');
                style.textContent = `
                    .sfq-sound-gate {{
                        position: fixed; inset: 0; z-index: 999999;
                        background: radial-gradient(circle at 50% 40%, rgba(20,24,38,0.97), rgba(8,10,18,0.99));
                        display: flex; align-items: center; justify-content: center;
                        cursor: pointer; transition: opacity 0.3s ease;
                        font-family: 'Space Grotesk', sans-serif;
                    }}
                    .sfq-sound-gate-inner {{
                        text-align: center; color: #f5f3ea; padding: 2rem;
                        border: 1px solid rgba(232,199,102,0.35); border-radius: 20px;
                        background: rgba(255,255,255,0.03);
                    }}
                    .sfq-sound-gate-icon {{ font-size: 3rem; margin-bottom: 0.6rem; animation: sfq-gate-pulse 1.6s ease-in-out infinite; }}
                    @keyframes sfq-gate-pulse {{ 0%,100% {{ transform: scale(1); }} 50% {{ transform: scale(1.08); }} }}
                    .sfq-sound-gate-title {{
                        font-family: 'Fraunces', serif; font-weight: 600; font-size: 1.4rem;
                        color: #e8c766; letter-spacing: 0.02em;
                    }}
                    .sfq-sound-gate-sub {{ margin-top: 0.5rem; font-size: 0.9rem; color: rgba(245,243,234,0.65); }}
                `;
                topDoc.head.appendChild(style);

                const gate = topDoc.createElement('div');
                gate.id = 'sfq-sound-gate';
                gate.className = 'sfq-sound-gate';
                gate.innerHTML = `
                    <div class="sfq-sound-gate-inner">
                        <div class="sfq-sound-gate-icon">🔊</div>
                        <div class="sfq-sound-gate-title">Tap to Enable Sound</div>
                        <div class="sfq-sound-gate-sub">Serene Falah Quiz uses sound effects and music</div>
                    </div>
                `;
                topDoc.body.appendChild(gate);

                gate.addEventListener('click', function unlock() {{
                    try {{
                        topWin.__sfqSoundUnlocked = true;
                        // Unlock HTML5 Audio autoplay (needed for ticktick,
                        // which is the only looping/auto-triggered audio
                        // left in the app) with a silent dummy play/pause.
                        const dummy = new topWin.Audio();
                        dummy.play().catch(() => {{}});
                        dummy.pause();
                        // Also resume WebAudio in case any synthesized
                        // fallback tone needs it.
                        const ctx = new (topWin.AudioContext || topWin.webkitAudioContext)();
            ...
            console.log('Sound gate setup skipped (cross-origin restriction?):', e);
        }}
    }})();
    </script>
    """
...
def render_display():
    ...
    # Must use components.html, not st.markdown -- <script> tags inside
    # st.markdown(unsafe_allow_html=True) are never executed by Streamlit.
    # This only unlocks the browser's autoplay permission (needed for
    # ticktick); it never plays any specific track on its own -- see
    # render_sound_unlock_overlay's docstring.
    components.html(render_sound_unlock_overlay(), height=0, width=0)
...
def _render_live_game_area():
    ...
    # Admin "Test" button previews: broadcast the same way as real cues so
    # both the display screen and the admin panel hear the preview.
    last_preview = st.session_state.get("last_preview_id", 0)
    current_preview_id = state.get("preview_id", 0)
    if current_preview_id != last_preview and state.get("preview_file"):
        play_file_preview(state["preview_file"], current_preview_id, state.get("preview_loop_seconds"))
        st.session_state["last_preview_id"] = current_preview_id

    # Splash screen: shown before the host begins Question 1. No audio
    # plays automatically here anymore -- the host plays the intro exactly
    # once via the "▶️ Play Intro" button in the admin panel (that button
    # broadcasts a one-shot preview, picked up by the preview-listener
    # above, and then hides itself after being pressed).
    if status == "lobby":
        st.markdown(render_splash_screen(), unsafe_allow_html=True)
        return

    # Continuous soft ambient sound (ticktick) while the countdown timer is
    # actually running -- NOT as soon as options start appearing. It should
    # begin the instant option D appears and the timer auto-starts (see
    # advance_auto_reveal_and_timer), and stop once the timer's done or the
    # question is locked/revealed/answered.
    timer_is_running = (
        state.get("timer_started_at") is not None
        and timer_remaining(state) > 0
    )
    ambient_should_play = (
        status == "question"
    ...
    if status in ("question", "locked", "revealed"):
        if q_index < len(bank):
            q = bank[q_index]
            # The "suspense" visual pulse on the question card now runs for
            # the same window as the suspense audio loop -- from the moment
            # the question is shown until the answer is locked in.
            in_suspense = status == "question" and state.get("question_revealed", False)
            card_class = "sfq-question-card suspense" if in_suspense else "sfq-question-card"
            card_class += animate_class

            question_is_revealed = state.get("question_revealed", False)
            ...
            if question_is_revealed:
                ...
                # Host hasn't pressed "Show Question" yet -- show an empty
                # waiting card instead of the question text.
                st.markdown(
                    f'<div class="{card_class}"><div class="sfq-question-text sfq-question-waiting">'
                    f'Get ready...</div></div>',
                    unsafe_allow_html=True,
                )
...
def render_admin():
    ...
    # The admin panel is a separate browser tab/window from the display
    # screen, so it needs its own "Tap to Enable Sound" unlock -- the
    # display screen's unlock only covers ITS OWN window.top, not this one.
    components.html(render_sound_unlock_overlay(), height=0, width=0)

    state = get_state()
    status = state["status"]

    # Play preview sounds (from the Audio Files "Test" buttons below) on the
    # admin panel too, using the admin's own last-played tracker in
    # session_state so it doesn't interfere with the display screen's copy.
    # Real in-game cues (lock, wrong answer, etc.) are intentionally NOT
    # played here -- only the host-triggered previews -- so the admin panel
    # doesn't double up sound with whatever's happening on the live TV screen.
    last_preview_admin = st.session_state.get("admin_last_preview_id", 0)
    current_preview_id = state.get("preview_id", 0)
    if current_preview_id != last_preview_admin and state.get("preview_file"):
        play_file_preview(state["preview_file"], current_preview_id, state.get("preview_loop_seconds"))
        st.session_state["admin_last_preview_id"] = current_preview_id

    # Auto-refresh the admin panel (slower than the display's 0.35s -- this
    # page has many buttons, and a host doesn't need sub-second precision,
    # just enough to watch the countdown timer without manually refreshing).
    # Options are now revealed by individual host clicks (Show A/B/C/D), so
    # only the timer itself auto-progresses and needs polling here.
    auto_progressing = state.get("timer_started_at") is not None and timer_remaining(state) > 0
    ...
    with st.expander("🎬 New Game Setup", expanded=(status == "lobby")):
        ...
        if status == "lobby" and not state.get("intro_played", False):
            st.write("")
            if st.button("▶️ Play Intro", use_container_width=True, type="primary"):
                # One-shot only -- trigger_preview plays it once on both the
                # display and admin screens, no looping, and this button
                # disappears immediately after (won't reappear until the
                # next "Start New Game" resets intro_played to False).
                trigger_preview(CUSTOM_AUDIO_FILES.get("lobby_intro", ""))
                set_state({"intro_played": True})
                st.rerun()

    with st.expander("🎵 Audio Files", expanded=False):
        st.caption(
            "Upload your custom sound for each cue below. Files save straight "
            "to this app's assets folder and are ready to use immediately -- "
            "no GitHub, no redeploy. Note: on Streamlit Cloud this storage is "
            "temporary, so re-upload here if the app restarts (e.g. before a "
            "live show)."
        )
        for cue_key, filename, label in ALL_AUDIO_SLOTS:
            file_state, size = audio_file_status(filename)
            col1, col2 = st.columns([3, 2])
            with col1:
                if file_state == "ok":
                    st.markdown(f"**{label}**  \n✅ Loaded ({size / 1024:.0f} KB)")
                elif file_state == "broken":
                    st.markdown(f"**{label}**  \n⚠️ File exists but is broken ({size} bytes) — please re-upload")
                else:
                    st.markdown(f"**{label}**  \n➖ Not uploaded yet (using default tone)")
            with col2:
                uploaded = st.file_uploader(
                    "Upload", type=["mp3"], key=f"upload_{cue_key}", label_visibility="collapsed"
                )
                # file_uploader keeps returning the same UploadedFile across
                # reruns until the widget is cleared, so we hash its bytes
                # to make sure we only write it to disk once instead of
                # re-saving on every unrelated rerun (Test button clicks,
                # auto-refresh, etc.). Hashing avoids depending on internal
                # attributes that vary across Streamlit versions.
                if uploaded is not None:
                    file_bytes = uploaded.getvalue()
                    content_hash = hash(file_bytes)
                    if st.session_state.get(f"saved_{cue_key}_hash") != content_hash:
                        written = save_uploaded_audio(filename, uploaded)
                        st.session_state[f"saved_{cue_key}_hash"] = content_hash
                        if written < 1024:
                            st.error(f"Upload failed — only {written} bytes received. Try again.")
                        else:
                            st.success(f"Saved {filename} ({written / 1024:.0f} KB)")
                            st.rerun()  # refresh status line + pick up the cache-busted file immediately
                if file_state == "ok":
                    if st.button("🔊 Test", key=f"test_{cue_key}", use_container_width=True):
                        loop_seconds = 3 if cue_key in ("ticktick", "lobby_intro") else None
                        trigger_preview(filename, loop_seconds)
                        st.rerun()
        st.divider()
        st.caption(
            "Cues without an uploaded file automatically fall back to the "
            "built-in synthesized tones (suspense, correct-answer chimes, "
            "lifelines, and the final win fanfare always use these, since no "
            "custom file is mapped for them)."
        )
    ...
    if status == "lobby":
        ...
        if st.button("▶️ Begin Question 1", use_container_width=True, type="primary"):
            ...
            if fresh.get("active_question_bank"):
                ...
                # one-shot that marks the show actually kicking off. The
                # question itself stays hidden (question_revealed=False)
                # until the host presses "Show Question" below.
                trigger_sound("start_play", {"status": "question", "question_revealed": False})
                st.rerun()
            else:
    ...
    if status == "question" and not question_is_revealed:
        st.write("**📋 Question is hidden on the display screen.**")
        if st.button("👁️ Show Question", use_container_width=True, type="primary"):
            set_state({"question_revealed": True})
            st.rerun()
        return  # nothing else to do until the question itself is shown

    if status == "question":
        st.write("**👁️ Reveal each option individually** — clicking D also starts the timer:")
        opt_cols = st.columns(4)
        for i in range(4):
            with opt_cols[i]:
                already_shown = i in revealed_options
                if st.button(
                    f"Show {'ABCD'[i]}", key=f"reveal_opt_{i}",
                    disabled=already_shown, use_container_width=True,
                    type="secondary" if already_shown else "primary",
                ):
                    new_revealed = sorted(set(revealed_options) | {i})
                    if i == 3:
                        # Showing D is also the cue that kicks off the
                        # countdown timer -- advance_auto_reveal_and_timer
                        # (called on the next state read) handles starting
                        # it once it sees all 4 revealed.
                        trigger_sound("question", {"revealed_options": new_revealed})
                    else:
                        set_state({"revealed_options": new_revealed})
                    st.rerun()
                if already_shown:
                    st.caption("✅ shown")
            ...
            with rcol2:
                if st.button("➡️ Next Question", use_container_width=True):
                    ...
                    if not is_correct:
                        ...
                        # plays next_question.mp3 (your custom cue). The
                        # question stays hidden (empty box only) until the
                        # host presses "Show Question" -- see question_revealed.
