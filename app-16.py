...
def play_ambient_loop(active: bool, tension: str = "low"):
    """Continuous soft background pad while a question is live, using
    persistent low-volume detuned oscillators (a 'pad' sound) rather than
    one-shot notes. Re-invoking this with active=True is a no-op if a loop
    is already running (tracked via a global on window), so it doesn't
    restart/click every 0.35s poll tick. tension affects pitch/brightness:
    'low' during Q1-5, 'mid' during Q6-10, 'high' during Q11-15 (subtle,
    ambient -- not meant to be loud or distracting).
    """
    tension_settings = {
        "low": {"base": 110, "detune": 4, "gain": 0.035},
        "mid": {"base": 146, "detune": 6, "gain": 0.045},
        "high": {"base": 196, "detune": 9, "gain": 0.055},
    }
...
def _render_live_game_area():
    ...
    # Continuous soft ambient pad while a question is actively being played
    # (not during lobby, suspense, or after reveal/win/lose -- those get
    # their own one-shot cues instead). play_ambient_loop is idempotent, so
    # calling it every 0.35s tick doesn't restart or click the audio.
