...
if st.session_state.account is None:
    ...
    with login_col_mid:
        ...
        # Reliable fallback that doesn't depend on cross-origin JS working:
        # a plain Streamlit text input + button. If the boxes above ever
        # fail to auto-submit (blocked window.top/parent access on some
        # hosting setups), this always works.
        with st.expander("Having trouble? Enter your code here instead"):
            fallback_code = st.text_input(
                "4-digit passcode", max_chars=4, key="login_fallback_code",
                label_visibility="collapsed", placeholder="1234"
            )
            if st.button("Submit code", key="login_fallback_submit"):
                if fallback_code and fallback_code.strip().isdigit() and len(fallback_code.strip()) == 4:
                    otp_code = fallback_code.strip()
                else:
