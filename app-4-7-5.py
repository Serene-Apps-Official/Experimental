...
def render_welcome_animation(display_name):
    """Full-width welcome moment shown right after a successful login,
    reusing the same confetti/celebrate-card visual language as
    render_celebration() so it feels native to the rest of the app."""
    st.markdown(f"""
    <div class="celebrate-wrap">
        <div class="celebrate-confetti">
            {''.join(f'<span class="confetti-piece c{i % 6}"></span>' for i in range(28))}
        </div>
        <div class="celebrate-card" style="padding:2.4rem 1.4rem;">
            <div class="celebrate-emoji" style="font-size:3.2rem;">📚✨</div>
            <div class="celebrate-title" style="font-size:1.7rem; margin-top:0.4rem;">Welcome, {display_name}!</div>
            <div class="celebrate-sub">Great to see you — let's get you signed in.</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
        ...
        (function() {{
            ...
            function submitIfComplete() {{
                ...
                if (code.length === {num_boxes}) {{
                    let done = false;
                    // Streamlit Cloud serves the app inside a nested,
                    // cross-origin-ish frame structure. window.parent is
                    // not always the actual browser tab — window.top is.
                    // Try top first, then parent, and never fail silently.
                    try {{
                    ...
                    if (!done) {{
                        let fallback = document.getElementById('otp_fallback_{widget_key}');
                        if (!fallback) {{
                            fallback = document.createElement('div');
                            fallback.id = 'otp_fallback_{widget_key}';
                            fallback.style.cssText = 'text-align:center; margin-top:10px; font-size:0.85rem; color:#b3261e;';
                            document.body.appendChild(fallback);
                        }}
                        fallback.textContent = 'Could not auto-submit — please copy this code and use the box below: ' + code;
                    }}
                }}
            }}
...
def do_logout():
    ...
    st.session_state.just_logged_in_name = None
...
if st.session_state.account is None:
    ...
    with login_col_mid:
        ...
        if st.session_state.get("login_error_for_name") != chosen_name:
            st.session_state.login_error = None
        st.session_state.login_error_for_name = chosen_name

        render_field_label("Enter your 4-digit passcode")
        passcode_input = st.text_input(
            "4-digit passcode", max_chars=4, key="login_passcode_input",
            label_visibility="collapsed", placeholder="1234", type="password"
        )
        submitted = st.button("Submit", use_container_width=True, key="login_submit_btn")

        otp_code = None
        if submitted:
            candidate = (passcode_input or "").strip()
            if not candidate.isdigit() or len(candidate) != 4:
                st.session_state.login_error = "Enter exactly 4 digits."
            else:
                otp_code = candidate
        ...
        if otp_code:
            ...
            if chosen_name == ADMIN_NAME:
                ...
                if not admin_pw:
                    ...
                    st.session_state.login_error = "Incorrect passcode."
                else:
                    st.session_state.login_error = None
                    ...
                    st.session_state.just_logged_in_name = ADMIN_NAME
                    st.rerun()
            else:
                ...
                if not expected_pw:
                    ...
                    st.query_params["t"] = token
                    save_token_to_local_storage(token)
                    st.session_state.just_logged_in_name = chosen_name
                    st.rerun()
...
# Show a one-time welcome animation immediately after a fresh login
# (set by the login block via just_logged_in_name), then require a tap
# to continue before the rest of the app renders. The flag is cleared
# the moment "Continue" is pressed so it never reappears on later
# reruns within the same session.
if st.session_state.get("just_logged_in_name"):
    welcome_col_l, welcome_col_mid, welcome_col_r = st.columns([1, 3, 1])
    with welcome_col_mid:
        render_welcome_animation(st.session_state.just_logged_in_name)
        if st.button("Continue", use_container_width=True, key="welcome_continue_btn"):
            st.session_state.just_logged_in_name = None
            st.rerun()
    st.stop()
...
# ---- Admin: Export & log ---------------------------------------------
    ...
    st.markdown('<div class="section-label">Full database backup</div>', unsafe_allow_html=True)
    st.caption(
        "The CSV above only has reservation details — it does not include "
        "student signatures. This backup is the complete database file "
        "(accounts, signatures, reservations, everything) exactly as it "
        "exists right now. Keep a copy somewhere safe every so often, "
        "since the app's storage isn't guaranteed to persist forever."
    )
    try:
        ...
        st.download_button(
            ...
            data=db_bytes,
            file_name=f"safha_backup_{datetime.date.today().isoformat()}.db",
