...
def create_reservation(book_id, student_name, needed_by_date):
    with get_conn() as conn:
        conn.execute(
            """INSERT INTO reservations
               (book_id, student_name, needed_by_date, status, created_at)
               VALUES (?, ?, ?, 'waiting', ?)""",
            (book_id, student_name, needed_by_date, now_iso())
        )
...
def format_timestamp_display(iso_str):
    """Formats a stored ISO timestamp like '5 August, 3:42 PM'."""
    if not iso_str:
        return "—"
    try:
...
def render_signature_canvas(widget_key):
    """Draws an HTML5 <canvas> the student can sign on with a mouse or a
    finger (touch). Pure browser HTML5 + JS — no package, no
    requirements.txt change. On "Use this drawing", the canvas is
    exported as a small PNG data URL and put directly into the page URL
    as ?sig_drawn=<data-url>, which Python reads back on the next rerun
    via st.query_params — the same bridge pattern used for login tokens.
    The canvas is kept small (500x180) specifically so the resulting PNG
    stays well under typical browser URL-length limits."""
    components.html(f"""
        <div style="font-family: 'Plus Jakarta Sans', sans-serif;">
            <canvas id="sigpad_{widget_key}" width="500" height="180"
                style="width:100%; max-width:500px; height:180px;
            ...
            <div style="margin-top:10px; display:flex; gap:10px;">
                <button id="clearbtn_{widget_key}" type="button" style="
                    flex:1; padding:10px; border-radius:999px; border:1.5px solid rgba(124,92,252,0.3);
...
if st.session_state.account is None:
    ...
    with login_col_mid:
        ...
        render_header("Shaikh Zulqarnain · 10th A", "Safha", "Select your name and enter your passcode.")

        render_field_label("Select your name")
        chosen_name = st.selectbox("Name", NAME_OPTIONS, label_visibility="collapsed")

        render_field_label("Passcode")
        passcode_input = st.text_input(
            "Passcode", type="password", label_visibility="collapsed", placeholder="Enter your passcode"
        )

        if st.button("Log in", use_container_width=True):
            passcode_clean = passcode_input.strip()
            if not passcode_clean:
                ...
                elif passcode_clean != expected_pw:
                    st.error("Incorrect passcode.")
                else:
                    synth_email = synthetic_email_for_name(chosen_name)
                    existing_acct = get_account_by_email(synth_email)
...
with nav_col:
    if is_admin:
        nav = st.radio(
            "Navigate",
            ["Overview", "Queues", "All reservations", "Students", "Activity", "Export & log"],
...
if not is_admin and not account.get("signature_data") and nav == "Reserve a book":
    ...
    MAX_SIGNATURE_BYTES = int(1.5 * 1024 * 1024)

    # Check first if a drawn signature is waiting from a previous rerun
    # (student clicked "Use this drawing" in the canvas below).
    drawn_b64, drawn_mime = read_drawn_signature("first")
    if drawn_b64:
        save_account_signature(account["email"], drawn_b64, drawn_mime)
        st.session_state.account = get_account_by_email(account["email"])
        st.success("Signature saved — you're all set.")
        st.rerun()

    sig_mode = st.radio("How would you like to sign?", ["Draw it", "Upload a photo"], horizontal=True, label_visibility="collapsed")

    if sig_mode == "Draw it":
        render_field_label("Draw your signature below")
        render_signature_canvas("first")
    else:
        render_field_label("Upload a photo of your signature (max 1.5MB)")
        ...
        if st.button("Save signature", use_container_width=True):
            if first_sig is None:
                ...
                st.error("That photo is over 1.5MB. Please upload a smaller image.")
            else:
    ...
    with st.form(key="reserve_form", clear_on_submit=True, border=False):
        ...
        if submitted:
            ...
            if student_already_in_queue(book_id, account["name"]):
                ...
                create_reservation(book_id, account["name"], needed_by.isoformat())
...
# ---- My reservations ---------------------------------------------------
    ...
    if not my_res:
        ...
        for r in my_res:
            ...
            reserved_display = format_timestamp_display(r["created_at"])
            st.markdown(f"""
            <div class="res-card">
...
# ---- My account (students only): view/replace signature on file ------
    ...
    st.markdown('<div class="row-spacer"></div>', unsafe_allow_html=True)
    render_field_label("Replace signature (max 1.5MB)")
    MAX_SIGNATURE_BYTES = int(1.5 * 1024 * 1024)

    drawn_b64_acct, drawn_mime_acct = read_drawn_signature("account")
    if drawn_b64_acct:
        save_account_signature(account["email"], drawn_b64_acct, drawn_mime_acct)
        st.session_state.account = get_account_by_email(account["email"])
        st.success("Signature updated.")
        st.rerun()

    sig_mode_acct = st.radio("How would you like to sign?", ["Draw it", "Upload a photo"], horizontal=True, label_visibility="collapsed", key="sig_mode_account")

    if sig_mode_acct == "Draw it":
        render_field_label("Draw your new signature below")
        render_signature_canvas("account")
    else:
        new_sig = st.file_uploader("New signature", type=["png", "jpg", "jpeg"], label_visibility="collapsed")
...
# ---- Admin: Queues -------------------------------------------------
    ...
    for item_type in SUBJECTS[pick_subject]:
        ...
        for r in queue:
            ...
            with c1:
                reserved_ts = format_timestamp_display(r["created_at"])
                st.markdown(f"**{r['student_name']}** · needed by {r['needed_by_date']} · reserved {reserved_ts}")
            with c2:
    ...
    if not all_res:
        ...
        for r in all_res:
            ...
            table_rows.append({
                ...
                "Reserved on": format_timestamp_display(r["created_at"]),
                "Status": r["status"],
        ...
        st.dataframe(
            ...
            hide_index=True,
            column_order=["Name", "Book Reserved", "Needed by", "Reserved on", "Status", "Returned"],
        )
        ...
        for r in all_res:
            ...
            with st.expander(f"{r['student_name']} — {subject} ({item_type}) · {date_display}"):
                ...
                if student_acct and student_acct.get("signature_data"):
                    try:
                        sig_bytes = base64.b64decode(student_acct["signature_data"])
                        st.image(sig_bytes, caption=f"{r['student_name']}'s signature on file", width=180)
                    except Exception:
...
# ---- Admin: Students -------------------------------------------------
    ...
    for a in accounts_list:
        ...
        with c1:
            ...
            last_login = format_timestamp_display(a.get("last_login_at")) if a.get("last_login_at") else "never logged in"
            sig_status = "signature on file" if a.get("signature_data") else "no signature yet"
            st.markdown(f"**{a['name']}** · {status} · {sig_status}  \nLast login: {last_login}")
        with c2:
            if a["suspended"]:
...
# ---- Admin: Activity (login + reservation timeline) -------------------
    ...
    render_header("Safha", "Activity", "Every login and reservation, timestamped.")
    ...
    if not logins:
        ...
        login_rows = [
            {"Name": l["account_name"], "Logged in at": format_timestamp_display(l["timestamp"])}
            for l in logins
        ]
    ...
    if not all_res_activity:
        ...
        for r in all_res_activity:
            ...
            res_rows.append({
                "Name": r["student_name"],
                "Book Reserved": f"{subject} — {item_type}",
                "Reserved at": format_timestamp_display(r["created_at"]),
                "Status": r["status"],
            })
...
# ---- Admin: Export & log ---------------------------------------------
    ...
    if all_res:
        ...
        import io
        buf = io.StringIO()
        fieldnames = ["Name", "Book Reserved", "Needed by", "Status", "Returned", "Returned On", "Reserved On"]
