...
CUSTOM_AUDIO_FILES = {
    ...
    # Correct answer (any prize tier) reuses the same intro_start_play.mp3
    # file per your request -- all three tiers point at the same audio.
    "correct_small": "intro_start_play.mp3",
    "correct_medium": "intro_start_play.mp3",
    "correct_big": "intro_start_play.mp3",
}
...
ALL_AUDIO_SLOTS = [
    ...
    ("start_play", "intro_start_play.mp3", "▶️ Begin Question 1 / Correct Answer"),
...
DEFAULT_STATE = {
    ...
    "options_reveal_started_at": None,  # wall-clock time host pressed "Show Options"; drives the 1.5s-apart auto-reveal
...
def advance_auto_reveal_and_timer(state: dict) -> dict:
    ...
    elapsed time since 'options_reveal_started_at' (1.5 seconds apart),
    and auto-starts the countdown timer the instant the 4th option
    ...
    elapsed = time.time() - started_at
    # Option i becomes visible once `elapsed` passes i * OPTION_REVEAL_GAP:
    # A at 0s, B at 1.5s, C at 3.0s, D at 4.5s.
    should_be_revealed = [i for i in range(4) if elapsed >= i * OPTION_REVEAL_GAP]
...
def render_sound_unlock_overlay():
    ...
    On tap, it plays-and-pauses a real Audio object and resumes a
    WebAudio AudioContext, which is what "unlocks" autoplay for the rest
    of that browser tab -- and because the flag (__sfqSoundUnlocked) and
    the gate element both live on window.top, they persist across every
    later components.html() call the fragment makes, even though each of
    those is a brand new, separate iframe.
    """
    return """
    <script>
    (function() {
...
def render_admin():
    ...
    if status == "question" and not reveal_in_progress:
        st.write(f"**👁️ Reveal options** — each appears {OPTION_REVEAL_GAP:.1f} seconds apart, automatically:")
