    ...
    if ticktick_uri:
        ...
        (function() {{
            try {{
                ...
                if (!topWin.__sfqTicktick) {{ topWin.__sfqTicktick = {{ audio: null, playing: false }}; }}
...
def play_lobby_intro_loop(active: bool):
    ...
    IMPORTANT: only calls components.html() when `active` actually
    CHANGES (tracked via st.session_state), not on every 0.35s poll tick --
    see play_ambient_loop's docstring for why re-embedding a multi-MB
    base64 blob every tick froze/darkened the page. Silently does nothing
    if the file hasn't been uploaded to assets/ yet (and doesn't mark
    itself as 'started' in that case, so uploading the file while already
    sitting on the lobby screen still triggers playback on the next tick
    ...
    (function() {{
        try {{
            const topWin = window.top || window;
            const AudioCtor = topWin.Audio || window.Audio;
            if (!topWin.__sfqLobbyIntro) {{ topWin.__sfqLobbyIntro = {{ audio: null, playing: false }}; }}
...
def play_suspense_loop(active: bool):
    ...
    Same pattern as play_ambient_loop / play_lobby_intro_loop: only calls
    components.html() when `active` actually changes, and the actual
    <audio> element + its state live on window.top so they survive across
    the many separate iframes Streamlit's fragment reruns create."""
    ...
    (function() {{
        try {{
            const topWin = window.top || window;
            const AudioCtor = topWin.Audio || window.Audio;
            if (!topWin.__sfqSuspense) {{ topWin.__sfqSuspense = {{ audio: null, playing: false }}; }}
...
def render_sound_unlock_overlay(lobby_intro_uri: str = None):
    ...
    if lobby_intro_uri:
        play_intro_js = f"""
                        if (!topWin.__sfqLobbyIntro) {{ topWin.__sfqLobbyIntro = {{ audio: null, playing: false }}; }}
                        if (!topWin.__sfqLobbyIntro.playing) {{
