...
SUSPENSE_LOOP_FILE = "suspense.mp3"          # loops from question-shown until locked-in
...
ALL_AUDIO_SLOTS = [
    ("lobby_intro", "kbc_intro_main.mp3", "🏠 Lobby / Splash Screen Intro"),
    ("start_play", "intro_start_play.mp3", "▶️ Begin Question 1"),
    ("suspense", "suspense.mp3", "🥁 Suspense (loops: question shown → locked)"),
...
DEFAULT_STATE = {
    ...
    "reveal_stage": None,  # None | "done" -- "suspense" was retired; see the continuous suspense audio/visual loop instead
    "eliminated_options": [],
...
.sfq-question-text.sfq-question-waiting {
    color: var(--sfq-ink-dim); font-style: italic; font-size: 1.2rem;
...
def play_lobby_intro_loop(active: bool):
    ...
    IMPORTANT: st.session_state is NOT a reliable guard here -- a
    WebSocket reconnect (common on flaky venue wifi, or just Streamlit
    Cloud's normal connection cycling) gives the browser tab a brand new,
    empty session_state, even though it's the same tab/window with the
    same window.top. If we only tracked "did this change" in
    session_state, a reconnect would look like a fresh transition and
    re-trigger components.html() -> a second Audio object playing over
    the first, which is what caused the intro to play multiple times
    simultaneously. So the ONLY source of truth for "is it already
    playing" is window.top.__sfqLobbyIntro.playing, checked inside the
    JS itself, every single call, regardless of whether Python thinks
    this is a new state or not. We still skip re-embedding the full
    base64 blob when nothing changed (for the performance reasons in
    play_ambient_loop's docstring), but the JS is written so that even if
    it DOES run again, it can never create a second overlapping Audio."""
    intro_uri = _load_audio_b64(CUSTOM_AUDIO_FILES.get("lobby_intro", ""))
    if not intro_uri:
        return

    prev_active = st.session_state.get("_lobby_intro_active", False)
    if active == prev_active:
        return
    st.session_state["_lobby_intro_active"] = active
    ...
    (function() {{
        try {{
            const topWin = window.top || window;
            const AudioCtor = topWin.Audio || window.Audio;
            if (!topWin.__sfqLobbyIntro) {{ topWin.__sfqLobbyIntro = {{ audio: null, playing: false, starting: false }}; }}
            const state = topWin.__sfqLobbyIntro;
            const shouldPlay = {str(active).lower()};
            ...
            // state.playing covers "already looping"; state.starting closes
            // the tiny race window between checking and actually setting
            // playing=true, so two near-simultaneous calls (e.g. this
            // function firing right after the tap-to-unlock overlay
            // already started the same track) can never both pass the
            // guard and create two overlapping Audio objects.
            if (state.playing || state.starting) return;
            state.starting = true;

            const audio = new AudioCtor("{intro_uri}");
            audio.loop = true;
            audio.volume = 0.75;
...
def play_suspense_loop(active: bool):
    """Loops suspense.mp3 from the moment the question is shown (host
    presses "Show Question") until the answer is locked in -- gives the
    audience a building-tension bed under the whole thinking period,
    independent of the ticktick timer sound (which only starts once
    option D is revealed and the countdown begins).

    IMPORTANT: st.session_state alone is NOT a reliable guard for "is this
    already playing" -- a WebSocket reconnect (flaky wifi, Streamlit
    Cloud's normal connection cycling) hands the same browser tab a brand
    new, empty session_state even though window.top is unchanged. If we
    only trusted session_state's "did this change" check, a reconnect
    mid-question would look like a fresh start and spawn a second,
    overlapping copy of the loop -- audibly indistinguishable from the
    track "restarting nonstop". So the actual source of truth for
    "already playing" is topWin.__sfqSuspense.playing/.starting, checked
    fresh inside the JS every single time this runs, regardless of what
    Python's session_state believes. The session_state check below is
    purely a performance shortcut (skip re-embedding the base64 blob when
    nothing changed) -- it is never the thing preventing a double-play."""
    suspense_uri = _load_audio_b64(SUSPENSE_LOOP_FILE)
    if not suspense_uri:
        return

    prev_active = st.session_state.get("_suspense_active", False)
    if active == prev_active:
        return
    st.session_state["_suspense_active"] = active
    ...
    (function() {{
        try {{
            const topWin = window.top || window;
            const AudioCtor = topWin.Audio || window.Audio;
            if (!topWin.__sfqSuspense) {{ topWin.__sfqSuspense = {{ audio: null, playing: false, starting: false }}; }}
            const state = topWin.__sfqSuspense;
            const shouldPlay = {str(active).lower()};
            ...
            // .playing covers "already looping"; .starting closes the race
            // window between checking and setting playing=true, so two
            // near-simultaneous calls can never both create an Audio object.
            if (state.playing || state.starting) return;
            state.starting = true;

            const audio = new AudioCtor("{suspense_uri}");
            audio.loop = true;
            audio.volume = 0.6;
            audio.play().catch(e => console.log('Suspense autoplay blocked:', e));
            state.audio = audio;
            state.playing = true;
            state.starting = false;
        }} catch (e) {{
            console.log('Suspense loop skipped:', e);
        }}
    }})();
    </script>
    """
...
def render_sound_unlock_overlay(lobby_intro_uri: str = None):
    ...
    On tap: if lobby_intro_uri is provided (i.e. we're on the lobby/splash
    screen and the file's been uploaded), the tap DIRECTLY starts that
    exact track -- this is the one and only place the main intro begins
    playing, right at the moment of the tap, rather than relying on a
    separate autoplay attempt a moment later. If no lobby_intro_uri is
    given (gate showing outside the lobby, e.g. after a mid-game refresh),
    the tap still unlocks audio generally via a silent dummy Audio object,
    it just doesn't start any specific track.
    ...
    if lobby_intro_uri:
        play_intro_js = f"""
                        if (!topWin.__sfqLobbyIntro) {{ topWin.__sfqLobbyIntro = {{ audio: null, playing: false, starting: false }}; }}
                        if (!topWin.__sfqLobbyIntro.playing && !topWin.__sfqLobbyIntro.starting) {{
                            topWin.__sfqLobbyIntro.starting = true;
                            const introAudio = new topWin.Audio("{lobby_intro_uri}");
                            introAudio.loop = true;
                            introAudio.volume = 0.75;
                            introAudio.play().catch(e => console.log('Intro play-on-tap blocked:', e));
                            topWin.__sfqLobbyIntro.audio = introAudio;
                            topWin.__sfqLobbyIntro.playing = true;
                            topWin.__sfqLobbyIntro.starting = false;
                        }}
        """
    return f"""
    <script>
...
def render_display():
    ...
    # Only pass the lobby intro's audio in if we're actually on the lobby
    # screen -- that's the one and only place the tap-to-unlock gesture
    # should double as "start the main intro track".
    current_status = get_state().get("status")
    lobby_intro_uri = _load_audio_b64(CUSTOM_AUDIO_FILES.get("lobby_intro", "")) if current_status == "lobby" else None
    components.html(render_sound_unlock_overlay(lobby_intro_uri), height=0, width=0)
...
def _render_live_game_area():
    ...
    # Continuous suspense loop: starts the instant the question is shown
    # (host presses "Show Question") and runs the whole thinking period,
    # stopping once the answer is locked in. This is separate from the
    # existing one-shot "🥁 Build Suspense" drumroll cue, which still fires
    # after lock-in as a final dramatic beat before the reveal.
    suspense_loop_should_play = (
        status == "question"
        and state.get("question_revealed", False)
    )
    play_suspense_loop(suspense_loop_should_play)
...
def render_admin():
    ...
    if status == "question" and not question_is_revealed:
        ...
        if st.button("👁️ Show Question", use_container_width=True, type="primary"):
            # Suspense starts the instant the question itself is shown, and
            # keeps looping until the answer is locked in (see play_suspense_loop).
