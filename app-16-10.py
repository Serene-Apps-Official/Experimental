...
/* ---- Responsive sizing for TV / tablet: uses clamp() so text and
   spacing shrink gracefully on shorter screens. Note: this does NOT
   force a hard single-viewport height anymore -- an earlier attempt at
   that (forcing overflow:hidden + a flex column on .block-container)
   fought with Streamlit's own nested DOM structure and caused the whole
   layout to visually collapse. clamp()-based sizing alone gets most
   screens close to a single view without that risk. ---- */
.block-container {
