...
.kbc-title {
    text-align: center; font-size: 2.1rem; font-weight: 800; letter-spacing: 0.08em;
...
.kbc-subtitle {
    text-align: center; color: #ffd76a; font-size: 1rem; font-weight: 600;
    margin-bottom: 1.1rem; letter-spacing: 0.03em; min-height: 1.4rem;
}

/* ---- Header bar: title/name on left, ladder toggle button on right ---- */
.kbc-header-row {
...
@keyframes kbc-slide-in-option {
    from { opacity: 0; transform: translateX(-24px); }
    to { opacity: 1; transform: translateX(0); }
}
.kbc-question-card.animate-in {
    animation: kbc-slide-up 0.55s cubic-bezier(0.22, 1, 0.36, 1) both;
}
.kbc-option.animate-in {
    animation: kbc-slide-in-option 0.45s cubic-bezier(0.22, 1, 0.36, 1) both;
}
/* stagger each option's entrance slightly for a cascading feel */
.kbc-option.animate-in.opt-0 { animation-delay: 0.05s; }
.kbc-option.animate-in.opt-1 { animation-delay: 0.15s; }
.kbc-option.animate-in.opt-2 { animation-delay: 0.25s; }
.kbc-option.animate-in.opt-3 { animation-delay: 0.35s; }
...
.kbc-ladder {
    ...
    animation: kbc-slide-up 0.4s cubic-bezier(0.22, 1, 0.36, 1) both;
}
.kbc-ladder-row {
...
/* ---- Milestone panel: current amount badge always visible, full ladder toggled ---- */
.kbc-milestone-badge {
    background: linear-gradient(135deg, rgba(10, 16, 45, 0.92), rgba(5, 8, 25, 0.96));
    border: 1.5px solid rgba(255, 183, 71, 0.4); border-radius: 14px;
    padding: 0.55rem 1rem; text-align: center; min-width: 150px;
}
.kbc-milestone-badge .label {
    font-size: 0.65rem; color: #9fb3d9; letter-spacing: 0.08em; text-transform: uppercase;
}
.kbc-milestone-badge .amount {
    font-size: 1.15rem; font-weight: 800; color: #ffd76a;
}
...
def render_display():
    ...
    st.markdown('<div class="kbc-title">SERENE FALAH QUIZ</div>', unsafe_allow_html=True)
...
def _render_live_game_area():
    ...
    show_options = state.get("show_options", False)
    ...
    # Detect whether the question/options actually changed since the last
    # poll tick, so the slide-in animation fires once on a genuine
    # transition rather than replaying every 0.35s the fragment reruns.
    transition_key = (status, q_index, show_options)
    ...
    with header_col2:
        ...
        st.markdown(
            f'<div class="kbc-milestone-badge"><div class="label">Playing For</div>'
            f'<div class="amount">{current_prize}</div></div>',
            unsafe_allow_html=True,
        )
    with header_col3:
        label = "🏆 Hide Ladder" if st.session_state["show_ladder_panel"] else "🏆 Milestones"
    ...
    if status in ("question", "locked") and show_options:
